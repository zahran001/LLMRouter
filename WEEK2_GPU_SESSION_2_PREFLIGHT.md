...
| | |
|---|---|
| Branch | Evidence gathered on `week2/loadgen-baseline`; **merged to `main` before the session**. `run_on_instance.sh` pins to whatever `HEAD` is when it runs, so the branch it sat on is provenance, not a constraint |
...
| Item | Result |
|---|---|
| Documents classified | **15** live Week 2 process documents, in `WEEK2_DOC_INDEX.md`; the 8 dead ones were deleted 2026-08-20 |
| Index completeness | **PASS** — no unindexed markdown at repo root or in `docs/`; new files fail closed |
| Current GPU runbooks | **1** — `WEEK2_GPU_SESSION_2_PLAN.md` |
...
|---|---:|---|
| `AUTHORITATIVE` | 6 | `STATUS.md`, `WEEK2_PLAN.md`, `WEEK2_EXECUTION.md`, `WEEK2_DOC_INDEX.md`, `docs/WEEK2_MOCK_VALIDATION.md`, `repeat_policy.json` |
| `EXECUTABLE` | 2 | `WEEK2_GPU_SESSION_2_PLAN.md` **(the runbook)**, `GPU_SESSION_NOTES.md` (setup reference) |
...
| 3 | `WEEK2_GPU_SESSION_2_PLAN.md` §5 | Offered an `N = 5,000` escalation — **contradicts lock 2B**. Removed |
| 4 | `WEEK2_GPU_SESSION_2_PLAN.md` §6 | Scout fallback ladder read "λ 0.5, 0.25" and "λ 16, 32" — **contradicts lock 5A**. Now 0.5 and 16 only |
| 5 | `WEEK2_GPU_SESSION_2_PLAN.md` §6 | "The preemption question this plan cannot resolve alone" — resolved by lock 3A |
| 6 | `WEEK2_GPU_SESSION_2_PLAN.md` §8 | Steady + adversarial listed as out of scope — lock 6A keeps them in Week 2 |
| 7 | `WEEK2_GPU_SESSION_2_PLAN.md` header | `Status: PROPOSED` → `EXECUTABLE` |
...
`repeat_policy.json` (machine-readable), `WEEK2_DOC_INDEX.md` §5 (index),
`WEEK2_GPU_SESSION_2_PLAN.md` (operational).
...
walks `README.md` → `STATUS.md` → `WEEK2_DOC_INDEX.md` → {`WEEK2_PLAN.md`,
`WEEK2_EXECUTION.md`, `WEEK2_GPU_SESSION_2_PLAN.md`, `repeat_policy.json`}
...
```
README.md → STATUS.md → WEEK2_DOC_INDEX.md
authority:     WEEK2_PLAN.md, WEEK2_EXECUTION.md
GPU execution: WEEK2_GPU_SESSION_2_PLAN.md
